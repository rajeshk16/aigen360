'use strict';function buttonLoader(){return`<svg class="loader animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" width="72" height="72" viewBox="0 0 72 72" fill="none">
        ...
        <linearGradient id="paint0_linear_1032_3036" x1="46.8123" y1="63.1382" x2="21.8195" y2="6.73779" gradientUnits="userSpaceOnUse">
            ...
            <stop offset="1" stop-color="#FFCF4B"></stop>
...
</svg>`}$(document).ready(function(){function a(a,c,d){a=a.toString(),c=c.toString();var e=b(a),f=b(c),g=parseFloat(a.replace(e,".")),h=parseFloat(c.replace(f,".")),i=(g-h).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2}).replace(".",d);return i}function b(a){return a=a.toString(),-1===a.indexOf(",")?-1===a.indexOf(".")?".":".":","}$(".apply-button").on("click",function(){var c=$(".coupon-input").val(),d=$("#currency_name").text(),e=$(".plan-price").text();""!==c&&($(".apply-loader").removeClass("hidden").html(buttonLoader),$.ajax({url:check_coupon,type:"POST",data:{_token:CSRF_TOKEN,code:c,package_id:$("#package_id").val(),billing_cycle:$("#billing_cycle").val()},success:function(c){if("success"==c.status){var f=``;for(const g in c.data)f+=`<div class="parent-flex"><div class="flex justify-between items-center pt-[14px]">
                            ...
                            <span class="cursor-pointer remove-coupon">
                                ...
                                </span></p><p class="text-[#DF2F2F]">-${a(c.data[g],"0",b(e))} <span>${d}</span></p></div></div>`,e=a(e,c.data[g],b(e));$(".discount-price").text(e),$("#coupon-list").html(f),toastMixin.fire({title:c.message,icon:"success"})}else toastMixin.fire({title:c.message,icon:"error"});$(".coupon-input").val("")},complete:function(){$(".apply-loader").addClass("hidden").html("")}}))}),$(document).on("click",".remove-coupon",function(){var c=$(this).closest(".parent-flex").find(".coupon-code").text(),d=this,e=$(".plan-price").text();$(this).html(buttonLoader),$.ajax({url:reset_discount,type:"POST",data:{_token:CSRF_TOKEN,code:c,package_id:$("#package_id").val(),billing_cycle:billing_cycle},success:function(c){e=a(e,c.amount.toString(),b(e)),$(".discount-price").text(e),$(d).closest(".parent-flex").remove()}})}),$(".checkout-btn").on("click",function(){$(this).find("svg").removeClass("hidden"),setTimeout(()=>{$(this).find("svg").addClass("hidden")},6e3)})});
