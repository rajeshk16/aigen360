...
if ($('.main-body .page-wrapper').find('#language-settings-container').length) {
    ...
    $(document).on('click', '.edit_language', function () {
        var id = $(this).attr("id");
        $.ajax({
            url: SITE_URL + "/languages/" + id + "/edit",
            type: 'GET',
            dataType: 'JSON',
...
if ($('.main-body .page-wrapper').find('#sso-settings-container').length) {
    $('.removeSpace').on('keyup', function(e){
         let data = $(this).val();
...
$(document).on('change', "#sso_google", function(e) {
    var googleCheck = e.target.checked;

    if (!googleCheck) {
        $('.google-client-id-label, .google-client-secret-label').removeClass('require')
        $('.google-client-id, .google-client-secret').removeAttr('required style');
        $('.google-client-id, .google-client-secret').attr('oninvalid',`this.setCustomValidity('')`);
        $('div.has-validation-error').removeClass('has-validation-error').find('.error').remove();
    } else {
        $('.google-client-id-label, .google-client-secret-label').addClass('require')
        $('.google-client-id, .google-client-secret').prop('required', true);
        $('.google-client-id, .google-client-secret').attr('oninvalid',`this.setCustomValidity('${jsLang('This field is required')}')`);
    }
})

$(document).on('change', "#sso_facebook", function(e) {
    var facebookCheck = e.target.checked;

    if (!facebookCheck) {
        $('.facebook-client-id-label, .facebook-client-secret-label').removeClass('require')
        $('.facebook-client-id, .facebook-client-secret').removeAttr('required style');
        $('.facebook-client-id, .facebook-client-secret').attr('oninvalid',`this.setCustomValidity('')`);
        $('div.has-validation-error').removeClass('has-validation-error').find('.error').remove();
    } else {
        $('.facebook-client-id-label, .facebook-client-secret-label').addClass('require')
        $('.facebook-client-id, .facebook-client-secret').prop('required', true);
        $('.facebook-client-id, .facebook-client-secret').attr('oninvalid',`this.setCustomValidity('${jsLang('This field is required')}')`);
    }
})
...
$('#customer_signup, #welcome_email').on('change', function() {
    if ($(this).val() == "0") {
        $(this).attr('value', '1');
    } else {
        $(this).attr('value', '0');
...
$("#is_redirect_link").on("change", function () {
    if ($(this).val() == "1") {
        $(".redirect_link_div").attr("hidden", true);
        $(this).attr("value", "0");
        $("#redirect_link").attr("required", false);
    } else {
        $(".redirect_link_div").attr("hidden", false);
        $(this).attr("value", "1");
        $("#redirect_link").attr("required", true).attr('name', "redirect_link");
    }
})

$("#is_default_package").on("change", function () {
    if ($(this).val() == "1") {
        $(".signup_email_div").attr("hidden", true);
        $(this).attr("value", "0");
        $("#features_word, #features_image, #features_minute, #features_character, #name").attr("required", false);
    } else {
        $(".signup_email_div").attr("hidden", false);
        $(this).attr("value", "1");
        $("#features_word, #features_image, #features_minute, #features_character, #name").attr("required", true);
    }
})
