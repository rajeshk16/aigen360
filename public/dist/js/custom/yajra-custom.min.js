'use strict'; $("#dataTableBuilder_length").addClass("p-0"), $("#dataTableBuilder").removeAttr("style"), $(document).on("click", ".filterbtn", function () { $(this).toggleClass("btn-primary active") }), $(document).on("change", ".filter", function () { var a = "?"; $(".filter").each(function () { a += $(this).attr("name") + "=" + $(this).val() + "&" }), $(`#dataTableBuilder`).DataTable().ajax.url(a).load() }); const characters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"; function generateString(a) { let b = ""; const c = characters.length; for (let d = 0; d < a; d++)b += characters.charAt(Math.floor(Math.random() * c)); return b } function headerCheckBox() {
    ...
    let a = "", b = 0; $(".need-batch-operation").find("table tbody tr").each((c, d) => {
        a = generateString(10), b = $(d).find(".confirm-delete").attr("data-id"), $(d).prepend(`
