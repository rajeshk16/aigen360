...
@section('logo', asset(moduleConfig('paytm.logo')))

@section('gateway', moduleConfig('paytm.name'))
...
@section('content')
    ...
    <form class="pay-form needs-validation"
        action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => moduleConfig('paytm.alias')])) }}" method="post"
        id="payment-form">
        @csrf
        <div>
            <div id="card-element">
        ...
        <button type="submit" class="pay-button sub-btn">{{ __('Pay With Paytm') }}</button>
    </form>
@endsection
