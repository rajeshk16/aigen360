...
<li class="{{ languageDirection() == 'ltr' ? 'ms-lg-3' : 'me-lg-3' }} nav-parent">
    <a class="d-flex align-items-center text-decoration-none hover_line" href="{{ auth()->user()->role()->slug == 'admin' ? route('admin.affiliate.dashboard') : route('site.affiliate.dashboard') }}" target="_blank">
                    <span>
                        <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg" class="external-link neg-transition-scale">
        ...
        <span class="{{ languageDirection() == 'ltr' ? 'ms-2' : 'me-2' }} list-curent-color">{{ __('Affiliate Panel') }}</span>
    </a>
</li>
@endif
