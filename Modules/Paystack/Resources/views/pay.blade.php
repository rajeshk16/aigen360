...
@section('logo', asset(moduleConfig('paystack.logo')))

@section('gateway', moduleConfig('paystack.name'))

@section('content')
    <p class="para-6">{{ __('Fill in the required information') }}</p>
    <div class="straight-line"></div>
    @include('gateway::partial.instruction')
    <form action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => moduleConfig('paystack.alias')])) }}"
