...
@section('logo', asset(moduleConfig('striperecurring.logo')))

@section('gateway', moduleConfig('striperecurring.name'))
...
@section('content')
    ...
    <form class="pay-form needs-validation"
        action="{{ route('gateway.complete', withOldQueryIntegrity(['gateway' => moduleConfig('striperecurring.alias')])) }}" method="post"
        id="payment-form">
        @csrf
        <div>
